...
I first converted the units of fish length from m to mm so the values wouldn't be fractions. I then converted fish weight from kg to g for the same reason. I then decided to look at the relationship between species and year. This table shows the mean length and weight of each of the 12 species with lengths and weights recorded from 2018-2023.
```{r}
...
species_lgt <- fish_mm %>% 
  group_by(common_name) %>% 
  summarize(mean_length_mm = mean(length_in_mm)) 
View(species_lgt)
  
#Graph
species_lgt %>% 
  ggplot(aes(common_name, mean_length_mm)) + 
  geom_point() + 
  ylim(0, 50) +
...
#Graph
dis_lgt %>% 
  ggplot(aes(district, mean_length_mm)) +
  geom_point() + 
  ylim(0, 50) +
